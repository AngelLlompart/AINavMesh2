...
public class State
{
    ...
    public enum STATE
    {
        PATROL, ATTACK, HIDE, RUNAWAY, FOLLOW
    };
...
public class Patrol : State
{
    ...
    public override void Update()
    {
        ...
        if (Vector3.Distance(player.position, npc.transform.position) > distance)
        {
...
public class Hides : State
{
    ...
    public override void Update()
    {
        agent.SetDestination(obstacleDesti.transform.position +
                             Vector3.Normalize(obstacleDesti.transform.position - player.position) * agent.speed);
